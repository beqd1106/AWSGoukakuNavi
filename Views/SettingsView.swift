    ...
    private var nextCertCard: some View {
        Card {
            VStack(alignment: .leading, spacing: Theme.Space.s) {
                ...
                certRow("Solutions Architect Associate", "設計者向け・次の登竜門", true)
                certRow("Developer Associate", "開発者向け", true)
                certRow("SysOps Administrator Associate", "運用者向け", true)
                Text("合格後はこれらの資格へ拡張予定です（現在はCloud Practitionerに対応）。")
                    .font(.system(size: 11)).foregroundStyle(Theme.inkSoft)
            }
        }
    }
    ...
    private var legalCard: some View {
        Card {
            VStack(alignment: .leading, spacing: Theme.Space.s) {
                ...
                bullet("本アプリの問題・解説はすべてオリジナルです。AWS公式問題・市販教材の転載はありません。")
                bullet("試験範囲・配点・受験要件は変更されることがあります。最新情報は必ずAWS公式サイトでご確認ください。")
                bullet("合格可能性スコアは学習の目安であり、合否を保証・予測するものではありません。")
                bullet("AWS、Amazon Web Services は Amazon.com, Inc. またはその関連会社の商標です。本アプリは非公式の学習支援アプリです。")
            }
        }
    }
